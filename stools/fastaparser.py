...
def loader(in_file, from_assembler):
    if os.path.isfile(in_file):
        ...
        df = data_wrangle(df, record_dict, from_assembler)
...
def data_wrangle(df, record_dict, from_assembler):
    if from_assembler == "yes":
        ...
        for key in record_dict:
            df = df.append({"Scaffold": record_dict[key].name  , "Length" : len(record_dict[key]) }, ignore_index=True)
    if len(df) > 0:
        return df
